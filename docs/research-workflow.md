...
**Isolated uv projects** — each Python experiment gets its own `uv init` →
`uv add <deps>` → `uv run` project under `experiments/<name>/`, with a
`pyproject.toml` + `uv.lock`. Missing imports are installed and the run retried
automatically.
